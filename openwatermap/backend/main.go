...
func main() {
	...
	r.Route("/api", func(r chi.Router) {
		...
		// Авторизация — строгий лимит (10 попыток/мин)
		r.Route("/auth", func(r chi.Router) {
			r.With(handlers.RateLimitAuth).Post("/register", authHandler.Register)
			r.With(handlers.RateLimitAuth).Post("/login",    authHandler.Login)
		...
		r.Route("/points", func(r chi.Router) {
			// Публичное чтение с опциональным токеном
			r.With(handlers.OptionalAuthMiddleware).Get("/",     waterHandler.GetPoints)
			...
			// Добавить точку — ДЛ-1, ДЛ-2, Админ
			r.Group(func(r chi.Router) {
				r.Use(handlers.AuthMiddleware)
				r.Use(handlers.RequireRole(
					models.RoleAdmin, models.RoleDL1, models.RoleDL2,
				))
			...
			// Просмотр и верификация заявок — только ДЛ-1 и Админ
			r.Group(func(r chi.Router) {
				r.Use(handlers.AuthMiddleware)
				r.Use(handlers.RequireRole(models.RoleAdmin, models.RoleDL1))
				r.Get("/pending",         waterHandler.GetPendingPoints)
				r.Post("/{id}/review",    waterHandler.ReviewPoint)
			})

			// Удалить — только Админ и ДЛ-1
			r.Group(func(r chi.Router) {
				r.Use(handlers.AuthMiddleware)
				r.Use(handlers.RequireRole(models.RoleAdmin, models.RoleDL1))
